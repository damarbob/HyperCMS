...
<div class="block">
    ...
    <?php if (!$eligibleModelExists || !$eligibleAssetsModelExists): ?>
        <div class="notification">
            <p><?= lang('PagingSystem.toEnsureBestExperienceInPageEditor') ?></p>
            <ul>
                <?php if (!$eligibleModelExists): ?>
                    <li>
                        <span class="icon">
                            <i class="fas fa-file"></i>
                        </span>
                        <span>
                            <?= lang('PagingSystem.page') ?>
                        </span>
                    </li>
                <?php endif ?>
                <?php if (!$eligibleAssetsModelExists): ?>
                    <li>
                        <span class="icon">
                            <i class="fas fa-file-code"></i>
                        </span>
                        <span>
                            <?= lang('PagingSystem.assets') ?>
                        </span>
                    </li>
                <?php endif ?>
            </ul>
            <a class="button is-primary mt-2" href="<?= base_url('admin/settings/paging-system?setup=true') ?>">
                <span class="icon">
                    <i class="fas fa-check"></i>
                </span>
                <span>
                    <?= lang('PagingSystem.oneClickSetup') ?>
                </span>
            </a>
        </div>
    <?php endif ?>

    <?php if ($eligibleModelExists || $eligibleAssetsModelExists || $eligibleMetaModelExists): ?>
        <form action="<?= base_url('admin/settings/paging-system/update') ?>" method="POST">
            ...
            <?php if ($eligibleMetaModelExists): ?>
