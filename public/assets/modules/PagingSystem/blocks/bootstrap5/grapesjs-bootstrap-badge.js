grapesjs.plugins.add('grapesjs-bootstrap-badge', function (editor, opts = {}) {
    const defaults = { classes: ['text-bg-primary', 'text-bg-secondary', 'text-bg-success', 'text-bg-danger', 'text-bg-warning', 'text-bg-info', 'text-bg-light', 'text-bg-dark'], ...opts };

    editor.Components.addType('bs-badge', {
        isComponent: el => el.tagName === 'SPAN' && el.classList.contains('bs-badge'),
        model: {
            defaults: {
                type: 'text',
                name: 'Badge',
                tagName: 'span',
                classes: ['bs-badge', 'badge', 'text-bg-primary'],
                droppable: true,
                editable: true,
                textable: 1,
                traits: [
                    {
                        type: 'select',
                        label: 'Type',
                        name: 'badgeType',
                        options: defaults.classes.map(cls => ({ value: cls, name: cls.replace('text-bg-', '') })),
                        changeProp: true,
                    },
                    {
                        type: 'checkbox',
                        label: 'Pill',
                        name: 'badgePill',
                        changeProp: true,
                    },
                ],
                components: [
                    {
                        type: 'text',
                        tagName: 'span',
                        content: 'Badge text',
                    },
                ],
                badgeType: 'text-bg-primary',
                badgePill: false,
            },
            init() {
                this.on('change:badgeType', () => {
                    const type = this.get('badgeType');
                    this.removeClass(defaults.classes.join(' '));
                    this.addClass(type);
                });
                this.on('change:badgePill', () => {
                    const pill = this.get('badgePill');
                    if (pill) {
                        this.addClass('rounded-pill');
                    } else {
                        this.removeClass('rounded-pill');
                    }
                });
            },
        },
    });

    editor.Blocks.add('bs-badge', {
        label: 'Badge',
        category: 'Bootstrap',
        content: {
            type: 'bs-badge',
        },
        media: '<svg aria-hidden="true" width="24" height="50" focusable="false" data-prefix="fas" data-icon="certificate" class="svg-inline--fa fa-certificate fa-w-16" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="currentColor" d="M458.622 255.92l45.985-45.005c13.708-12.977 7.316-36.039-10.664-40.339l-62.65-15.99 17.661-62.015c4.991-17.838-11.829-34.663-29.661-29.671l-61.994 17.667-15.984-62.671C337.085.197 313.765-6.276 300.99 7.228L256 53.57 211.011 7.229c-12.63-13.351-36.047-7.234-40.325 10.668l-15.984 62.671-61.995-17.667C74.87 57.907 58.056 74.738 63.046 92.572l17.661 62.015-62.65 15.99C.069 174.878-6.31 197.944 7.392 210.915l45.985 45.005-45.985 45.004c-13.708 12.977-7.316 36.039 10.664 40.339l62.65 15.99-17.661 62.015c-4.991 17.838 11.829 34.663 29.661 29.671l61.994-17.667 15.984 62.671c4.439 18.575 27.696 24.018 40.325 10.668L256 458.61l44.989 46.001c12.5 13.488 35.987 7.486 40.325-10.668l15.984-62.671 61.994 17.667c17.836 4.994 34.651-11.837 29.661-29.671l-17.661-62.015 62.65-15.99c17.987-4.302 24.366-27.367 10.664-40.339l-45.984-45.004z"></path></svg>'
    });
});